...
const nameBibl=['React','Vue','Express','Bootstrap']
const languages = [
    "JavaScript",
    "Python",
    "HTML",
...
const Quiz = () => {
    ...
    <label key={item}>
        <input type="radio" name='question1' value={item} checked={item ===answer1} onChange={changeAnswer1}/>
        {item}
        </label>
    )
    const showCheckbox=(item)=>(
        <label key={item}>
            <input type="checkbox" value={item} checked={answer2.includes(item)} onChange={changeAnswer2}/>
            {item}
        </label>
    )
...
setScore(result)
    ...
    return (
        <div>
            <fieldset>
                <legend>1) Яка бібліотека використовується для створення інтеріейсів?</legend>
          {nameBibl.map(showRadio)}
            </fieldset>
            <fieldset>
                <legend>2) Якими мовами можна змінити стилі елементів веб-сторінки?</legend>
            ...
            <button onClick={checkResult}>Резутьтат:</button>
            {score!==null && <p>{score}/3</p>}
        </div>
    );
}
